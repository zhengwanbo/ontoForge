...
**面向智能体Agent的本体构建与知识图谱平台。**

ontoForge 将业务系统的的源数据、业务语义、本体模型和 Oracle Property Graph 串联起来，帮助团队从数据表出发，完成业务对象建模、字段映射、图谱部署、图数据查询以及面向场景的智能体技能配置。

> 当前实现面向 Oracle 数据库与制造质量追溯场景；其中 Property Graph 浏览依赖目标 Oracle 环境已部署相应图谱并具有数据字典读取权限。
